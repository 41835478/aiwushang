...
class RowAService
{
    ...
    public function index($row_id,$now_row_id,$flag,$user_id,$current_level)
    {
        $res=$this->setPromoteRecode($now_row_id,$flag,$user_id,$current_level);
        if($res){
            if($row_id){
                return $this->getSelfPrevInfo($now_row_id,$row_id);
            }
            return true;
        }
        return false;
    ...
    {
        $rowA=Rowa::where(['id'=>$rowId])->value('prev_id');
        if($rowA&&($num<=$aim_level)){
            $num++;
    ...
    public function getSelfPrevInfo($now_row_id,$aim_row_id)
    {
    ...
    public function updateDynasty($from_row,$to_row)//升级循环判断
    {
        $feeConfig=$this->feeConfig($to_row->current_generate);
            ...
            if($main){
                ...
                if($res2->promote_fee>=$feeConfig['promoteFee']){
                    ...
                    if($main1){
                        ...
                        if($res3){
                            return $this->getSelfPrevInfo($to_row->id,$up_row_id);
                        }
                    }
                    return false;
                }
    ...
    public function loopPoint()//开始三个盘进行循环
    {
        $order_id=$this->createOrder->createOrder();
    ...
    public function mainFunc($user_id,$from_id,$current_level,$redPacket)//向上级返红包
    {
        ...
        $from_login_name=User::where(['id'=>$from_id])->value('login_nmae');
        DB::beginTransaction();
        try{
     ...
     * @return array
     */
    public function feeConfig($dynasty)//$dynasty 第几代 通过当前代数得到要向上级交的升级费，上级的到的见点奖，上级的每个点的见点扣的费用
    {
        $date=array();
        if($dynasty==1){
            $date['promoteFee']=100;
